...
function SeccionAlertaStock() {
  ...
  const [modalIngresoMaterial, setModalIngresoMaterial] = useState(null);

  const cargar = useCallback(async () => {
    setCargando(true);
    const [ra, rm] = await Promise.all([
      fetchAuth("/materiales?bajoStock=true"),
      fetchAuth("/materiales"),
    ]);
    if (ra.ok) setLista(await ra.json());
    if (rm.ok) setMateriales(await rm.json());
    setCargando(false);
  }, []);
  ...
  const onGuardadoIngreso = async () => {
    setModalIngresoMaterial(null);
    await cargar();
  };
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">
          {cargando ? "Cargando…" : `${lista.length} material${lista.length !== 1 ? "es" : ""} en alerta`}
        </p>
        <button onClick={exportarExcel} disabled={lista.length === 0}
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {!cargando && lista.length === 0 && (
              <tr><td colSpan={7} className="text-center py-10 text-gray-300 text-sm">Sin materiales en alerta — todo el stock está por encima de su mínimo</td></tr>
            )}
            {lista.map((m) => (
              <tr key={m._id} className="hover:bg-gray-50/50 transition">
                <td className="px-5 py-3 font-mono text-xs text-gray-500">{m.codigo}</td>
                ...
                <td className="px-5 py-3 text-center text-gray-500">{m.stockMinimo} {m.unidad}</td>
                <td className="px-5 py-3 text-right">
                  <button onClick={() => setModalIngresoMaterial(m._id)}
      ...
      {modalIngresoMaterial && (
        <ModalIngreso
          ...
          materialInicial={modalIngresoMaterial}
          onClose={() => setModalIngresoMaterial(null)}
          onGuardado={onGuardadoIngreso}
        />
      )}
    </div>
  );
...
function SeccionRotacion() {
  ...
  const cargar = useCallback(async () => {
    setCargando(true);
    const r = await fetchAuth("/materiales/rotacion");
    if (r.ok) setLista(await r.json());
    setCargando(false);
  }, []);
  ...
  const fmtFecha = (d) => d ? new Date(d).toLocaleDateString("es-PE") : "—";

  return (
    <div className="space-y-5">
      <p className="text-sm text-gray-500">
        Ranking de repuestos por rotación: total despachado (egresos) en los últimos 12 meses, de mayor a menor.
      </p>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              ...
              <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Nombre</th>
              <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide hidden md:table-cell">Ubicación</th>
              <th className="text-right px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Cant. despachada (12m)</th>
              <th className="text-right px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">N° salidas</th>
              <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Último movimiento</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {!cargando && lista.length === 0 && (
              <tr><td colSpan={7} className="text-center py-10 text-gray-300 text-sm">Sin egresos de repuestos en los últimos 12 meses</td></tr>
            )}
            {lista.map((r, i) => (
              <tr key={r.material._id} className="hover:bg-gray-50/50 transition">
                <td className="px-5 py-3 text-center text-gray-400 font-semibold">{i + 1}</td>
                <td className="px-5 py-3 font-mono text-xs text-gray-500">{r.material.codigo}</td>
                <td className="px-5 py-3 font-medium text-gray-800">{r.material.nombre}</td>
                <td className="px-5 py-3 text-gray-500 hidden md:table-cell">{r.material.ubicacion?.nombre || "—"}</td>
                <td className="px-5 py-3 text-right font-semibold text-gray-800">{r.cantidadTotal} {r.material.unidad}</td>
                <td className="px-5 py-3 text-right text-gray-600">{r.movimientos}</td>
                <td className="px-5 py-3 text-gray-500">{fmtFecha(r.ultimoMovimiento)}</td>
              </tr>
            ))}
          </tbody>
        </table>
