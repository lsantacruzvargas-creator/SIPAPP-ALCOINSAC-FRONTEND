...
function FilaItem({ requerimiento, item, puedeAtender, puedeDevolver, onActualizado }) {
  ...
  return (
    <div className="border-t border-gray-50 first:border-t-0 py-2.5">
      <div className="flex items-center gap-3">
        ...
        {puedeDevolver && item.estado === "atendido" && item.movimientoAlmacen && (
...
export default function Requerimientos() {
  ...
  const puedeDevolver = ["admin", "supervisor", "asistente"].includes(usuario?.rol);
          ...
          <div key={r._id} className="bg-white border border-gray-100 rounded-2xl shadow-sm p-5">
            ...
            <div>
              {itemsAMostrar.map((item) => (
                <FilaItem key={item._id} requerimiento={r} item={item} puedeAtender={puedeAtender} puedeDevolver={puedeDevolver}
